...
function showApp() {
  ...
  connection.textContent = state.localMode ? 'Local preview' : 'Cloudflare connected';
  root.querySelectorAll('[data-view]').forEach((button) => {
...
function renderQueue() {
  ...
  content.innerHTML = `${viewHeader(
    'Posts / working queue',
    'The desk queue.',
    'Search, filter, edit, and archive every story in the live Cloudflare edition.',
...
function renderEditor(post = {}) {
  ...
  const bodyLabel = value.sourceFormat === 'html' ? 'Migrated HTML body' : 'Markdown body';
  const bodyHelp =
    value.sourceFormat === 'html'
      ? 'This migrated story is stored as HTML. Keep its existing tags intact when editing the body.'
...
function renderRelease() {
  content.innerHTML = `${viewHeader('Publishing / direct', 'Publish once.', 'The desk validates and writes directly to Cloudflare. A successful story is live without a Prismic release or site rebuild.', '')}<section class="admin-release"><div class="admin-release-grid"><article class="admin-release-card"><span class="admin-kicker">01 / Review</span><strong>Check the story</strong><p>Confirm headline, author, dates, format, cover alt text, takeaways, facts tables, and source-linked body copy.</p></article><article class="admin-release-card"><span class="admin-kicker">02 / Validate</span><strong>Run the contract</strong><p>Canonical tags, required fields, image metadata, and heading structure must pass before any storage write.</p></article><article class="admin-release-card"><span class="admin-kicker">03 / Publish</span><strong>Write Cloudflare</strong><p>The body is versioned in R2 and its metadata, tags, search row, and publication event are committed to D1.</p></article><article class="admin-release-card"><span class="admin-kicker">04 / Index</span><strong>Request Google indexing</strong><p>Push every live post URL to Google's Indexing API after the story is verified.</p><button class="admin-button admin-button-primary" type="button" data-action="request-indexing">Request Google indexing</button><ul class="admin-indexing-results" data-indexing-result hidden></ul></article></div><div class="admin-editor-section"><h2>Credential boundary</h2><p class="admin-help">R2 access, the admin password hash, the session secret, and the Google Indexing API key live only in Worker secrets.</p></div></section>`;
}
