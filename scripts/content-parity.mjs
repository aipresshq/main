...
import { groupFieldToStrings, PRISMIC_LOCALE, PRISMIC_POST_TYPE, PRISMIC_REPOSITORY_NAME } from '../src/loaders/prismic-fields.ts';

const structured = new Set(['explainer', 'comparison', 'tracker', 'analysis', 'tutorial']);
const docs = (await prismic.createClient(PRISMIC_REPOSITORY_NAME).getAllByType(PRISMIC_POST_TYPE, { lang: PRISMIC_LOCALE })).filter((doc) => !doc.data.archived);
const expected = new Map();
for (const doc of docs) {
  const { html, headings } = serializeBodyWithHeadings(doc.data.body);
  const envelope = await createBodyEnvelope(html, 'html');
  const format = structured.has(doc.data.format) && headings.filter((heading) => heading.depth === 2).length < 2 ? 'brief' : doc.data.format;
...
const command = spawnSync('npx', ['wrangler', 'd1', 'execute', 'CONTENT_DB', '--remote', '--command', query, '--json'], { cwd: process.cwd(), encoding: 'utf8' });
...
for (const [id, source] of expected) {
  ...
  for (const field of ['title', 'description', 'pub_date', 'format', 'cover', 'body_hash']) {
    if (source[field] !== target[field]) mismatches.push({ id, field, expected: source[field], actual: target[field] });
  }
  const tags = JSON.parse(target.tags_json || '[]');
  if (JSON.stringify(source.tags) !== JSON.stringify(tags)) mismatches.push({ id, field: 'tags', expected: source.tags, actual: tags });
}
...
for (const row of actualRows) {
  ...
  } catch { missingBodies.push({ id: row.id, bodyKey: row.body_key }); }
}

const report = { sourcePublished: expected.size, targetPublished: actual.size, missing, unexpected, mismatches, missingBodies, passed: missing.length === 0 && unexpected.length === 0 && mismatches.length === 0 && missingBodies.length === 0 };
